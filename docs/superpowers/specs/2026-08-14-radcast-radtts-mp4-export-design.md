...
- replace the project cover only after a successful export and remove any obsolete `cover.<old-ext>` file when the replacement uses a different supported format; a failed or cancelled export leaves the prior project cover unchanged;
